...
- [#188](https://github.com/office-kit/xlsx/pull/188) [`2e80670`](https://github.com/office-kit/xlsx/commit/2e80670a986c79df762b3fd9c0e55c788c754aff) Thanks [@kibertoad](https://github.com/kibertoad)! - `setPrintArea` now writes a sheet-qualified value. It previously stored the `ref` verbatim, so `setPrintArea(wb, 0, 'A1:E20')` produced `_xlnm.Print_Area` = `A1:E20`, which Excel resolves against whatever sheet is active rather than the one the name is scoped to. The sheet title is taken from `sheetIndex` and always quoted, the way Excel writes built-in names, so `'A1:E20'` becomes `'Report'!A1:E20` and each leg of a multi-area range is qualified on its own. An already-qualified `ref` is stored as given.
...
- [#201](https://github.com/office-kit/xlsx/pull/201) [`5dfd6cf`](https://github.com/office-kit/xlsx/commit/5dfd6cf01216647e9829e4ef04482f9cbef210c9) Thanks [@kibertoad](https://github.com/kibertoad)! - `getDefinedNameTarget` no longer throws `missing "!" delimiter` on a sheet-scoped name whose value has no sheet prefix, such as `_xlnm.Print_Area` = `$A$1:$E$20`. Other tools, and earlier versions of `setPrintArea`, write print areas that way. An unqualified leg now resolves to the sheet the name is scoped to. A workbook-scoped name with an unqualified value still throws, since it names no sheet.

- [#200](https://github.com/office-kit/xlsx/pull/200) [`cff81df`](https://github.com/office-kit/xlsx/commit/cff81dfaec96503d1ea9dcaacbd1430729bb0111) Thanks [@kibertoad](https://github.com/kibertoad)! - `bindValue(cell, '=')` stores the text `=` instead of throwing, and `inferCellType('=')` returns `'s'`. Excel keeps a lone `=` typed into a cell as text; every other string that starts with `=` still takes the formula path, and the ones Excel refuses (`'= '`, `'==A1'`) still throw.
...
- [#176](https://github.com/office-kit/xlsx/pull/176) [`0e222ee`](https://github.com/office-kit/xlsx/commit/0e222ee11516f5aecfa4f4ea2e89d41de9f93ded) Thanks [@kibertoad](https://github.com/kibertoad)! - Stop telling the owner of a legacy `.xls` to decrypt it. Any OLE compound file was
  ...
  try {
    ...
    if (
      error instanceof OpenXmlUnsupportedFormatError &&
      error.format === "legacy-xls"
    ) {
...
- [#163](https://github.com/office-kit/xlsx/pull/163) [`1032347`](https://github.com/office-kit/xlsx/commit/1032347772f499bbcd8b15e8779ba63838050af3) Thanks [@kibertoad](https://github.com/kibertoad)! - Load the cell shapes a current Excel writes. A `<c t="e">` carrying one of the
...
- [#169](https://github.com/office-kit/xlsx/pull/169) [`42fff5c`](https://github.com/office-kit/xlsx/commit/42fff5ca709e4d12bb4ed9e5bcc04ca3c6059c46) Thanks [@kibertoad](https://github.com/kibertoad)! - Add `contentLimits` to `loadWorkbook` and `loadWorkbookStream`: a cap on the
...
- [#151](https://github.com/office-kit/xlsx/pull/151) [`3eb0ae8`](https://github.com/office-kit/xlsx/commit/3eb0ae8ae280f23398522b62ec5feaaf972bf8e4) Thanks [@baseballyama](https://github.com/baseballyama)! - Bound write-only shared-string retention for exports with many distinct plain
